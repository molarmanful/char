...
pm=setInterval(_=>{
  ...
  if(Object.getOwnPropertyNames(k).length){
    ...
    if(Die)
      ...
      //w
      if(k[87]&&(map[mR(pr-1)][pc]==floor||map[mR(pr-1)][pc]==field))
        pr=mR(pr-1);
      //s
      if(k[83]&&(map[mR(pr+1)][pc]==floor||map[mR(pr+1)][pc]==field))
        pr=mR(pr+1);
      //a
      if(k[65]&&(map[pr][mC(pc-1)]==floor||map[pr][mC(pc-1)]==field))
        pc=mC(pc-1);
      //d
      if(k[68]&&(map[pr][mC(pc+1)]==floor||map[pr][mC(pc-1)]==field))
        pc=mC(pc+1);
      //forcefield
      if(k[32])
        [...Array(7).keys()].map((a,b)=>(b=pr+b-3,[...Array(7).keys()].map((c,d)=>(d=pc+d-3,Up(b,d,field))))),
        score-=50;
    }
    ...
    if(die)
      ...
      //teleport
      if(r==50)
        er=pr+Math.random()*5|0+Math.random()*-2|0,
        ec=pc+Math.random()*5|0*-2|0,
        map=map.map((a,b)=>a.map((c,d)=>c==floor||c==wall||c==field?Math.random()*2|0&&Math.random()*2|0?wall:floor:c));
    }
    //enemy field death
